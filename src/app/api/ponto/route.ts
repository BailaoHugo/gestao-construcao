...
export async function GET(req: NextRequest) {
  try {
    ...
    const [trabRows, regRows, obraRows] = await Promise.all([
      pool.query(`SELECT id, nome, cargo, custo_hora AS "custoHora", ativo FROM trabalhadores WHERE ativo = true ORDER BY nome`),
      pool.query(
        `SELECT rp.id, rp.trabalhador_id AS "trabalhadorId", rp.obra_id AS "obraId",
                ...
                rp.notas, rp.tipo,
                t.nome AS "trabalhadorNome", o.name AS "obraNome", o.code AS "obraCode"
      ...
      pool.query(`SELECT id, code, name AS nome FROM obras WHERE estado = 'ativo' ORDER BY code`),
    ]);
    return NextResponse.json({ trabalhadores: trabRows.rows, registos: regRows.rows, obras: obraRows.rows, mes, dataInicio, dataFim });
  } catch (err) {
...
// POST /api/ponto
// Bulk: { bulk: [...], clearDates: [...] }
// Update: { id, obraId, horas, notas, tipo }
// Insert: { trabalhadorId, obraId, data, horas, notas, tipo }
  ...
  try {
    ...
    if (body.bulk !== undefined) {
      const { bulk, clearDates } = body as {
        bulk: { trabalhadorId: string; obraId?: string | null; data: string; horas: number; notas?: string | null; tipo?: string }[];
      ...
      try {
        ...
        for (const { trabalhadorId, data } of (clearDates ?? [])) {
          await client.query(`DELETE FROM registos_ponto WHERE trabalhador_id = $1 AND data = $2`, [trabalhadorId, data]);
        }
        let count = 0;
        for (const rec of bulk) {
          const { trabalhadorId, obraId, data, notas } = rec;
          const tipo = rec.tipo ?? 'normal';
          const horasNum = tipo === 'falta' ? 0 : Math.max(0, parseFloat(String(rec.horas ?? 8)) || 8);
          ...
          const custo = custoHora > 0 && horasNum > 0 ? custoHora * horasNum : null;
          await client.query(
            `INSERT INTO registos_ponto (trabalhador_id, obra_id, data, horas, custo, notas, tipo) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [trabalhadorId, obraId || null, data, horasNum, custo, notas || null, tipo]
          );
          count++;
        }
    ...
    const { id, trabalhadorId, obraId, data, horas, notas, tipo = 'normal' } = body;
    const horasNum = tipo === 'falta' ? 0 : Math.max(0, parseFloat(horas ?? '8') || 8);
    ...
    if (id) {
      ...
      const { rows } = await pool.query(
        `UPDATE registos_ponto SET obra_id=$1, horas=$2, custo=$3, notas=$4, tipo=$5, atualizado_em=now()
         WHERE id=$6
         RETURNING id, trabalhador_id AS "trabalhadorId", obra_id AS "obraId", data::text AS data, horas::float AS horas, custo::float AS custo, notas, tipo`,
        [obraId || null, horasNum, custo, notas || null, tipo, id]
      );
      return NextResponse.json(rows[0], { status: 200 });
    }

    if (!trabalhadorId || !data) return NextResponse.json({ error: 'trabalhadorId e data são obrigatórios' }, { status: 400 });
    const wRes = await pool.query(`SELECT custo_hora FROM trabalhadores WHERE id = $1`, [trabalhadorId]);
    const custoHora = wRes.rows[0] ? parseFloat(wRes.rows[0].custo_hora) : 0;
    const custo = custoHora > 0 && horasNum > 0 ? custoHora * horasNum : null;
    const { rows } = await pool.query(
      `INSERT INTO registos_ponto (trabalhador_id, obra_id, data, horas, custo, notas, tipo)
