...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    // If rows exist but all have valor_contrato = 0, delete and re-seed with correct values
    if (existing.rows.length > 0) {
      const allZero = existing.rows.every(r => parseFloat(r.valor_contrato) === 0);
      if (!allZero) {
      ...
      // Delete zero-value rows so we can re-seed with correct values
      await pool.query('DELETE FROM contrato_avancos WHERE contrato_id = $1', [contratoId]);
    }
    ...
    const linhasRes = await pool.query(
      `SELECT ordem,
              ...
              descricao,
              COALESCE(total_venda_linha, total_linha, quantidade * preco_venda_unitario, quantidade * preco_unitario, 0) AS valor_linha
    ...
    try {
      ...
      for (const linha of linhasRes.rows) {
        const ins = await c.query(
          ...
          [contratoId, linha.ordem, linha.capitulo || '', linha.descricao, parseFloat(linha.valor_linha) || 0]
        );
