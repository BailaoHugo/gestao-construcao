...
export async function getAccessToken(): Promise<string> {
  ...
  const resp = await fetch(oauthUrl + '/token', {
    ...
    body: new URLSearchParams({ grant_type: 'refresh_token', refresh_token: refreshToken, scope: 'commercial' }).toString(),
  });
  if (!resp.ok) { const text = await resp.text(); throw new Error('TOConline refresh falhou: ' + resp.status + ' ' + text); }
...
export async function tocFetch<T = unknown>(path: string): Promise<T> {
  ...
  const resp = await fetch(apiUrl + path, { headers: { Authorization: 'Bearer ' + token, Accept: 'application/json' } });
  if (!resp.ok) { const text = await resp.text(); throw new Error('TOConline API ' + resp.status + ': ' + text); }
  return resp.json();
}
...
export interface TocCentroCusto {
  id: string | number;
  code?: string; name?: string; nome?: string; active?: boolean;
}
...
export async function runFullSync(): Promise<SyncResults> {
  ...
  for (const [key, fn] of tasks) {
    try { results[key] = await fn(); } catch (e) { results[key] = { error: (e as Error).message }; }
  }
